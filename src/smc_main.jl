...
function smc(m,loglikelihood::Function, parameters::ParameterVector{U}, data::Matrix{S};
    ...
    @everywhere function mutation_closure(p::Vector{S}, d_μ::Vector{S}, d_Σ::Matrix{S}, n_free_para::Int,
                                          blocks_free::Vector{Vector{Int64}}, blocks_all::Vector{Vector{Int64}}, #n_free_para::Int,
                                          ϕ_n::S, ϕ_n1::S; c::S = 1.0, α::S = 1.0, n_mh_steps::Int = 1,
                                          old_data::T = Matrix{S}(undef, size(data, 1), 0)) where {S<:Float64, T<:Matrix}

        return mutation(loglikelihood, parameters, data, p, d_μ, d_Σ, n_free_para, blocks_free, blocks_all, #n_free_para,
                        ϕ_n, ϕ_n1; c = c, α = α, n_mh_steps = n_mh_steps, old_data = old_data,
